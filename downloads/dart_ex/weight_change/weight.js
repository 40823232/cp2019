//由dart2js（快速啟動發射器，強大）生成，Dart至JavaScript編譯器版本：2.4.0。
//代碼支持以下鉤子：
// dartPrint（message）：
//     如果定義了此函數，則會調用它而不是Dart [print]
//     方法。
//
// dartMainRunner（main，args）：
//     如果定義了此函數，則不會調用Dart [main]方法
//     直接。相反，將調用[main]及其參數的閉包
//     [args]傳遞給[dartMainRunner]。
//
// dartDeferredLibraryLoader（uri，successCallback，errorCallback）：
//     如果定義了此函數，則在延遲庫時將調用它
//     已加載。它應該加載並評估`uri`的javascript，然後調用
//     successCallback。如果這樣做失敗，則應使用以下命令調用errorCallback
//     錯誤。
//
// dartCallInstrumentation（id，qualifiedName）：
//     如果定義了此函數，則會在
//     方法或構造函數。僅在使用以下命令編譯程序時使用
//     --experiment-call-instrumentation。
{
}
（函數 dartProgram（）{
  函數 copyProperties（從，到）{
    var keys =  Object。鍵（來自）；
    為（VAR我=  0 ;我<  鍵。長度 ;我++）{
      var key = keys [i];
      to [key] = from [key];
    }
  }
  VAR  supportsDirectProtoAccess  =  函數（）{
    var  cls  =  function（）{
    };
    cls。原型 = {p ： {}};
    var object =  new  cls（）;
    如果（！（對象。__proto__  &&  對象。__proto__。p  ===  CLS。原型。p））
      返回 false ;
    嘗試 {
      如果（typeof運算 導航 ！=  “未定義”  &&  typeof運算 導航。的userAgent  ==  “串”  &&  導航。的userAgent。的indexOf（“鍍鉻/ ”）> =  0）
        返回 true ;
      如果（typeof運算版本==  “功能”  &&  版本。長度 ==  0）{
        var v =  版本（）;
        if（/ ^ \ d + \。\ d + \。\ d + \。\ d + $ /。測試（v））
          返回 true ;
      }
  ...
  函數 setFunctionNamesIfNecessary（持有人）{
    函數 t（）{
    }
    ;
    如果（typeof運算 牛逼。名稱 ==  “字符串”）
      回報 ;
...
;
  ...
  (function(callback) {
    ...
    初始化。currentScript  = currentScript;
    如果（typeof dartMainRunner ===  “ function ”）
      dartMainRunner（大號。主，[]）;
    其他
      大號。主要（[]）;
  }）;
